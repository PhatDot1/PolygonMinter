...
var createImageForData = async (indexOfNFTToMint, objectToMint) => {
  ...
  if (
    imageTemplate.bitmap.width != 1200 ||
    imageTemplate.bitmap.height != 1200 ||
    indexOfNFTToMint <= 0 ||
    objectToMint.programmeName == "" ||
    objectToMint.achievementLevel == ""
  ) {
  ...
  await printTextAtPosition(
    smallFont,
    objectToMint.programmeName.toUpperCase(),
    imageTemplate,
    screen_positions.LeftBottom,
    margin,
    imageTemplate.bitmap.width / 2,
    0,
    0
  );
  await printTextAtPosition(
    bigFont,
    objectToMint.achievementLevel.toUpperCase(),
    imageTemplate,
    screen_positions.LeftMiddle,
    margin,
    imageTemplate.bitmap.width,
    0,
    margin * 3.5
  );
  await printTextAtPosition(
